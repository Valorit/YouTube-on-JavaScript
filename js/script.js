...
function bindSlideToggle(trigger, boxBody, content, openClass) {
    ...
    button.element.addEventListener("click", () => {
        if (button.active === false) {
            ...
            box.classList.add(openClass); //добавляем активный класс меню для прозрачности
        } else {
            button.active = false;
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyAb0kCBFD9MZHE75lsuPnKEXYhNdH5ukmY',
        'discoveryDocs': ["https://www.googleapis.com/discovery/v1/apis/youtube/v3/rest"]
    }).then(function () {
        return gapi.client.youtube.playlistItems.list({
            "part": "snippet,contentDetails",
            "maxResults": '6',
            "playlistId": "PLbZdCxdJ5uH5zALLgwh5qfqmXkEJR_FyV"
        });
...
function createVideo() {
    ...
    tag.src = "https://www.youtube.com/iframe_api"; //api youtube
    var firstScriptTag = document.getElementsByTagName('script')[0];
    firstScriptTag.parentNode.insertBefore(tag, firstScriptTag); //вставить скрипт перед первым существующим скриптом
